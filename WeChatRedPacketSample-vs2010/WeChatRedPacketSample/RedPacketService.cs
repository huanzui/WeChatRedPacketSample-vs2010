...
namespace WeChatRedPacketSample
{
    public class RedPacketService
    {
        ...
        public RedPacketSentResult SendAsync(RedPacket request)
        {
            if (request == null)
            ...
            Yc.WeChat.O2O.Common.Log.WriteLog("DictionaryToXml："+xml);

            var result =  m_WeChatClient.PostAsync(xml);

            Yc.WeChat.O2O.Common.Log.WriteLog("m_WeChatClient.PostAsync：" + result);
